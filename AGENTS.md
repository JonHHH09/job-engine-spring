...
- `./scripts/run-local-mcp-http.sh` is the build-from-source dev path: it bootstraps the ignored `.env` (operator boundary on, private token), then runs `docker compose up -d --build --force-recreate --wait postgres mcp`, starting PostgreSQL privately and publishing MCP plus the operator routes only on host loopback; `python3 scripts/smoke-mcp-http.py` verifies initialize, discovery, and `health`. Use the raw `docker compose up -d --build --wait postgres mcp` when the environment file must not be touched.
- `./scripts/bootstrap-local-env.sh` creates/updates the git-ignored `.env`: it sets `JOB_ENGINE_OPERATOR_ENABLED=true` when the value is missing or `false`, and generates a 64-character `JOB_ENGINE_OPERATOR_BEARER_TOKEN` when the existing token is shorter than 32 characters. It is idempotent, never prints the token, and never overrides an explicitly chosen non-default value. `JOB_ENGINE_SKIP_ENV_BOOTSTRAP=true` disables the automatic call in the deployment scripts.
...
- `./scripts/run-release-mcp-http.sh ghcr.io/jonhhh09/job-engine-spring:vX.Y.Z` is the guarded release-only deployment path; it bootstraps `.env`, refuses local/`latest` images, and recreates the service without building.
...
- `scripts/tests/test-bootstrap-local-env.sh` is the Docker-free regression for the `.env` bootstrap (default-on, token strength, idempotence, explicit opt-out, no token in output).
...
- `adapter/in/http/operator` — privileged local operator MVC boundary, off by application default and enabled for local deployments through the ignored `.env`; it calls application use cases directly, never MCP adapters, and maps failures to RFC 9457 problem details (see the operator subsystem below).
...
- **Operator HTTP boundary** (`adapter/in/http/operator`) — privileged local MVC API. The application default is off (`job-engine.operator.enabled` falls back to `false`), but the local deployment scripts turn it on through `scripts/bootstrap-local-env.sh`, which writes `JOB_ENGINE_OPERATOR_ENABLED=true` and a generated `JOB_ENGINE_OPERATOR_BEARER_TOKEN` (at least 32 characters, or startup fails) into the git-ignored `.env`. Exposes `GET /api/operator/v1/ping`, `GET /api/operator/v1/health`, profile list/search/read/create/replace/delete under `/api/operator/v1/profiles`, `PATCH /api/operator/v1/profiles/{profileId}/projects/{projectId}`, `POST /api/operator/v1/profiles/pdf-ingestions`, `GET /api/operator/v1/profiles/{profileId}/pdf-source`, and the `GET /operator/` browser shell. Every route calls `ProfileService`/`ProfileSearchService`/`ProfilePdfIngestionService`/`ApplicationHealthService`, so MCP and HTTP share one validation and optimistic-concurrency gate. `OperatorSecurityFilter` runs first for every request: 404 while disabled, 400 on ambiguous/over-encoded operator paths, 403 on non-loopback host/peer or cross-origin, 401 with `WWW-Authenticate: Bearer` without the token. `OperatorPeerPolicy` decides the peer rule: loopback only on a host runtime, plus the container's own default gateway (read once from `/proc/net/route`) when `job-engine.mcp.containerized` is true, because Docker rewrites the source address of loopback-published traffic; sibling containers stay rejected and `X-Forwarded-*` is never trusted. `OperatorProblemHandler` maps `ApplicationErrorCode` to RFC 9457 problem details (`urn:job-engine:problem:<code>`). Jobs, matches, documents, and generation stay MCP-only — do not widen this surface without an explicit decision.
...
Keep the operator bearer token out of version control: `JOB_ENGINE_OPERATOR_ENABLED`
and `JOB_ENGINE_OPERATOR_BEARER_TOKEN` belong in the ignored `.env` file only, which
`scripts/bootstrap-local-env.sh` writes with a locally generated token. Never print,
log, or commit that token, never set a non-`false` default for
`job-engine.operator.enabled` in `application.yaml` or any committed file, and never
add a populated token to `.env.example`.
