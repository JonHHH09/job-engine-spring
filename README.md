...
`list_jobs`, `list_profiles`, and `list_match_reports` accept `limit` (default 20, maximum 100) plus an opaque versioned `cursor`. The cursor embeds the immutable `created_at, id` keyset, a first-page snapshot watermark, and a SHA-256 fingerprint of the tool scope and filters. Deleting the prior page's anchor or updating mutable display fields does not invalidate traversal; rows created after the first-page snapshot are intentionally excluded. Malformed cursors, cursors from another tool, and report cursors reused with different profile/job filters return a sanitized `validation_error`. Cursors are API tokens and clients must not parse or manufacture them.
