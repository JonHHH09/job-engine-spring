...
The GitHub Actions pipeline mirrors this split: pull requests to `development` run fast pipeline validation and unit tests, while trusted pushes to `development` plus manual CI runs also run the Docker-backed integration job (`./mvnw -Pintegration-tests verify`) and the container smoke job that verifies the real STDIO `tools/list` contract through `scripts/smoke-mcp-stdio.py`. Plain pushes to `master` run only a lightweight promotion guard that verifies `master` matches `origin/development` exactly instead of repeating the heavy `development` checks or accepting master-only changes. Tag releases remain explicit: pushing a `v*` tag runs clean full verification (`./mvnw clean -Drevision=<version> -Pintegration-tests verify`), copies the verified Maven jar to a versioned release artifact, builds the release container image from that same jar through `Dockerfile.release`, publishes `ghcr.io/<owner>/job-engine-spring:<tag>` and `:latest` for stable tags, records the published image digest, and uploads the jar, checksum, image ID, image digest, and verification reports as release artifacts; manual release workflow runs default to dry-run verification and do not publish unless run from a `v*` tag.
