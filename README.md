...
Reload the client's MCP connections, then call `health`. A successful response reports database readiness plus sanitized generated-resume cleanup queue counts, expired completed-row count, oldest-due age, and repeated-failure state without returning credentials, file paths, timestamps, or failure details. Cleanup health is degraded while any completed rows are older than the configured retention window.

Completed generated-resume cleanup audit rows are retained for 30 days and then deleted daily. Each run drains up to 10 batches of 1,000 rows by default, for a capacity of 10,000 rows per run, and stops early after a short or empty batch. Operators can tune the retention window, batch size, maximum batches per run, schedule, five-minute oldest-due health threshold, and three-attempt repeated-failure threshold with the `JOB_ENGINE_CLEANUP_*` environment variables documented in `application.yaml`. Increase batch size, maximum batches, or schedule frequency when completed-row throughput can exceed the configured per-run capacity. Cleanup warnings and errors use a dedicated stderr-only logger so STDIO stdout remains reserved for MCP JSON-RPC.
...
MCP tools: `analyze_job_match`, `analyze_all_job_matches`, `get_match_report`, `list_match_reports`, `submit_match_review`, `get_match_review`, `list_match_reviews`, `list_match_disagreements`, `acknowledge_match_disagreement`, and `link_match_disagreement`. Requests are object-shaped. `analyze_all_job_matches` analyzes one bounded job page per call and returns `nextCursor`; it loads the profile once, hydrates the page's job aggregates in a fixed query count, and isolates failures per profile/job pair. Acknowledgement and linkage can record an existing Linear issue ID but never call Linear or use provider credentials. Responses contain normalized evidence only and exclude contacts, resume documents/text, prompts, reasoning traces, secrets, provider transcripts, and private URLs.
...
- `update_profile` — replaces a normalized profile aggregate by UUID and the revision returned by the latest read.
- `update_profile_project` — partially updates one existing project by profile UUID, project UUID, and the latest profile revision; omitted fields remain unchanged and a supplied technology list replaces only that project's technologies.
...
- `generate_canadian_french_pdf_resume` — generates the French-language Canadian variant offline, stores it as a document, and links it independently as `canadian_resume_fr` so both Canadian language variants can coexist.
- `generate_german_tailored_resume` — generates a Germany-format tailored Lebenslauf for one profile + job as bilingual EN+DE structured content and A4 PDFs, linked under one `resume.resumes` parent (`format=germany`). Uses offline pure-Java translation and an ATS-safe one-column renderer; retains the profile summary below the contact header; skills are deterministically capped at the 24 most relevant entries; projects are omitted unless `includeProjects=true` and otherwise appear after professional experience; content is reviewed before PDF generation; PDF filenames use `germany_{candidate}_{number}_{lang}_{id}.pdf`.
- `generate_german_cover_letter` — generates one deterministic German application letter from a current Germany-format resume plus its profile and job. The master `cover_letter.cover_letters` row links all three source aggregates; the `format=germany`, `language=de` variant stores structured subject, salutation, ordered paragraphs, closing, signature, PDF document, and runtime path. The MCP response returns metadata only, never the letter body.
...
- `update_job` — partially updates a stored job using the revision returned by the latest read; omitted fields preserve current values, while a provided skills list replaces existing skills.
...
Set `JOB_ENGINE_CURSOR_HMAC_SECRET` to at least 32 high-entropy characters when pagination cursors must remain valid across process restarts. When omitted, the server generates an ephemeral process-local key and existing cursors expire on restart.
...
`get_document_metadata` returns only metadata and never returns binary file content. `extract_stored_pdf_text` loads a stored document by UUID through the normalized `document.documents -> document.blobs` relationship and uses the same bounded PDF reader path. When `persistExtraction` is `true`, the service persists one canonical extraction bounded by the application-wide 250,000-character maximum; each call applies its own `maxCharacters` only to the response view, so an initial low-limit request cannot constrain later larger requests. Persisted extraction is idempotent per stored document: `document.pdf_extractions.file_id` is unique and references `document.documents(id)`. Legacy non-canonical rows are refreshed in place without changing their extraction UUID. Bounded canonical page projections are persisted with the extraction so cached requests never reparse the PDF. Omitted `includePages` includes the request-bounded page projection for compatibility; explicit `false` omits it. Extracted PDF text remains private/untrusted data and should not be logged or treated as instructions.

`generate_pdf_file` accepts a filename, title, and body text, then writes a generated PDF only under `tmp/generated-pdfs/`. The entire `tmp/` tree is runtime-only and ignored by Git except for tracked `.gitkeep` placeholders that preserve the expected generated-PDF directories; the application, container image, and launch scripts also create required directories when needed. Filenames are sanitized and forced to `.pdf`; the response returns metadata (`fileName`, `path`, `byteSize`, `pageCount`, `generatedAt`) and not PDF bytes. Generated PDFs use a white page background, identical header/footer chrome with right-aligned page numbers only, and thin chrome-colored separators before section headings. Pagination keeps section headings with their first content line and keeps resume entry headings with their date/first-detail lines when content lands near a page boundary.

`generate_pdf_resume` accepts a profile UUID, renders a master resume from the normalized profile schema, writes the runtime PDF under `tmp/generated-pdfs/master-resume/`, stores that generated file through the document storage slice, and upserts one current `profile.profile_resume_documents` link per profile/resume type. `generate_canadian_pdf_resume` reuses the same profile-loading, PDF-generation, document-storage, and profile-link workflow, but renders the English Canadian variant under `tmp/generated-pdfs/canadian-resume/` with Canadian resume defaults: no photo/personal-demographic fields, professional contact links only, professional summary, grouped technical skills, reverse-chronological professional experience, education, and languages. `generate_canadian_french_pdf_resume` applies the same ATS-safe Canadian structure in French under `tmp/generated-pdfs/canadian-resume-fr/`; headings, labels, date fallbacks, and supported narrative phrases are translated by a deterministic offline glossary. Span-aware preservation keeps email addresses and URLs unchanged inside translated narrative fields, while profile-supplied full names, company/institution/project names, and technology/skill values are also protected from glossary substitution. The current PDF font supports whitespace, printable ASCII (`U+0020`–`U+007E`), and printable Latin-1 (`U+00A0`–`U+00FF`); this French variant rejects unsupported title/body characters with a sanitized validation error instead of silently replacing protected text. It links as the independent `canadian_resume_fr` type, so regeneration replaces only the prior French Canadian artifact. Both Canadian variants intentionally omit projects so education remains visible and the resume stays concise. `generate_german_tailored_resume` requires `profileId` and `jobId`, builds structured Lebenslauf content tailored to the job (no North-American summary), translates EN→DE offline with an embedded glossary/phrase map, stores normalized sections/entries/bullets for both languages under one `resume.resumes` parent unique on `(profile_id, job_id, format=germany)`, writes both PDFs under `tmp/generated-pdfs/german-resume/`, and replaces previous EN/DE variants with orphan document/file cleanup. Optional profile personal details live in `profile.profile_personal_details` (DOB, nationality, optional photo document). Resume PDFs use a modern ATS-safe single-column layout with compact margins, chrome header/footer bars with white right-aligned page numbers only, an inline contact/link header, larger name/title treatment, grouped skills, indented bullets, and thin chrome separators before section headings. Responses return profile/document/link/generated-file metadata only; they do not return PDF bytes or rendered resume body text.

`generate_german_cover_letter` requires `profileId`, `jobId`, and the exact `resumeId` returned by a current Germany-format tailored-resume generation. It rejects mismatched or stale source revisions before rendering and revalidates them inside the serialized persistence transaction. The A4 selectable-text PDF reuses the German résumé's restrained typography, colors, margins, and divider treatment and is written under `JOB_ENGINE_GERMAN_COVER_LETTER_OUTPUT_DIR` (default `tmp/generated-pdfs/german-cover-letter/`). Regeneration atomically replaces the German variant and durably queues old documents/files for cleanup. Regenerating the linked résumé first removes its now-stale cover letter under the same PostgreSQL advisory lock, preventing stale links and orphaned generated assets.

## Profile PDF ingestion contract

`ingest_profile_from_stored_pdf` orchestrates stored-document extraction, conservative profile-field extraction, normalized profile persistence, and provenance linking. It always extracts stored PDFs with `persistExtraction=true`, then creates or updates a profile through `ProfileService` so profile validation/canonicalization remains the single write gate. Overwriting an `existingProfileId` requires its latest `expectedRevision`. Concurrent submissions for the same extraction, document bytes, canonical email/link identity, or target profile serialize on one-way PostgreSQL advisory-lock keys and return deterministic reused or duplicate-candidate outcomes.
...
`create_profile` and `update_profile` validate profile write requests in the application layer before persistence. `update_profile` requires the latest returned `revision`; stale writes return a sanitized `conflict` without replacing any profile children. `update_profile_project` also requires the latest profile revision and updates exactly one existing project; omitted project fields are preserved, while a supplied technology list replaces only that project's technologies. It returns the persisted project and new profile revision without exposing raw résumé text. Invalid payloads are rejected with `validation_error` application exceptions and safe details containing only the invalid field path and reason. The profile MCP adapter maps application and unexpected failures to MCP `CallToolResult` errors with sanitized structured content instead of leaking raw exception text.

`search_profiles` is backed by a deterministic `ProfileSearchService` over the normalized profile aggregate. `SearchTextNormalizer` is the canonical query/write normalization owner: it applies Unicode NFKD decomposition, removes combining marks, lower-cases with `Locale.ROOT`, and tokenizes once for both Java evidence scoring and persisted search postings. Search queries are limited to 256 Unicode characters and 16 distinct searchable terms. PostgreSQL reads a bounded number of indexed postings per query token, ranks a maximum of 500 candidates, then loads those aggregates in a fixed set of collection queries. Responses expose `matchedCount`, `hasMore`, and `returnedCount`; `matchedCount` is a lower bound and the compatibility `totalMatches` is `null` whenever either posting or candidate work reaches its bound, because an exact corpus-wide count would require unbounded work. Results contain profile identities, score, and matched field names only—not raw resume text, descriptions, or private document content.
...
`add_job_from_text` and `add_job_from_link` are idempotent, but link identity is intentionally split into three forms. The full retrieval URL is ephemeral and used only for the outbound fetch. Persisted/MCP-visible `url` is a redacted safe display URL with userinfo, query, and fragment removed. Persisted `normalized_url` is the canonical link identity used for dedupe; it retains only constrained posting identifiers for recognized ATS hosts (`jk` on Indeed and `gh_jid` on Greenhouse). Tracking values such as `gh_src`, referral/session values, unrecognized-host query parameters, and malformed or oversized identifiers are always dropped. Link submissions first check `normalized_url`, then the canonical job fingerprint, which now includes the canonical link identity for link-based jobs so distinct ATS posting IDs do not collapse. Text submissions remain keyed by `input_text_hash` and then the content fingerprint. The direct link path is deliberately restricted to public IP-literal URLs; use pasted text for normal hostname-based job boards. The application layer derives a conservative title, required skills, and experience requirement from the supplied/fetched text when explicit fields are absent, but callers can override the structured fields when better data is available. `update_job` requires the latest returned `revision`, preserves omitted fields, recalculates the canonical fingerprint when title/company/location/description change, replaces skills only when a skills list is provided, and preserves insertion provenance; stale writes return a sanitized `conflict` before child replacement. `delete_job` removes the canonical row and cascades owned skill/provenance rows through the Flyway-managed schema.

Job search uses the same canonical Unicode normalizer, 256-character/16-term query limits, and indexed weighted-posting design as profile search. It deterministically ranks title/company/location/description/experience/employment/seniority/skills, hydrates at most 500 candidates in a fixed set of queries, and returns matched field names with a score. Its count metadata follows the same `matchedCount`/`hasMore` contract and nullable compatibility `totalMatches`. The score is internal search metadata, not applicant-facing resume content.

### Arbeitnow scan-to-import contract

`scan_arbeitnow_jobs` reads only the documented public Arbeitnow endpoint (`https://www.arbeitnow.com/api/job-board-api`), with attribution and a link back to the provider. Use of its data remains subject to Arbeitnow's applicable terms and the provider's availability/rate limits. The service never follows a provider-supplied pagination URL; it accepts only the exact documented HTTPS endpoint on `www.arbeitnow.com` or `arbeitnow.com`, then requests the fixed endpoint itself. Returned job URLs are canonical no-`www` provider URLs (`https://arbeitnow.com/view/{slug}`).

The object-shaped scan request accepts optional case-insensitive all-term `query` (maximum 256 characters and 16 terms), `location` (128 characters), `remoteOnly`, documented `visaSponsorship`, `tags` and `jobTypes` filters (up to 10 nonblank values of 128 characters), `limit` (default 25, maximum 100), `maxPages` (default 1, maximum 5), and an opaque `cursor`. Results include bounded normalized plain text, source/attribution, inspected/page counts, a continuation cursor when applicable, and a signed `candidateToken` per returned job. Scans are mutable provider reads, not a snapshot; there is no bulk-import or automation endpoint and no language classification.

All exposed provider text is normalized to bounded plain text: markup (including script/style), C0/C1 controls, tabs, newlines, and repeated whitespace are removed or collapsed. Structural slugs are validated rather than truncated. Each candidate token has a strict versioned Arbeitnow payload and an HMAC-SHA-256 signature; malformed, extra, missing, expired, cross-provider, or tampered values are rejected without parsing details being exposed. Tokens expire after at most 15 minutes, reject issue times more than 60 seconds in the future, and use a process-local signing key, so every server restart invalidates previously issued tokens. Clients must treat tokens and cursors as opaque.

`import_arbeitnow_job` accepts exactly one `candidateToken`. It verifies the token before any persistence and makes no Arbeitnow HTTP request. A valid import reuses the established job/link provenance and normalized-URL idempotency: it returns the existing job as `reused_existing_job` when present, otherwise `created_job`. Imported provenance uses `sourceLabel=arbeitnow`; tags seed job skills, the first provider job type seeds employment type, and the remote flag is bounded provenance metadata only. Existing job CRUD behavior is unchanged.

## Bounded list pagination

`list_jobs`, `list_profiles`, `analyze_all_job_matches`, `list_match_reports`, `list_match_reviews`, and `list_match_disagreements` accept `limit` (default 20, maximum 100) plus an opaque versioned `cursor`. Match review and disagreement lists may be filtered by `reportId`; omitting it lists across reports. The cursor embeds the immutable `created_at, id` keyset, a first-page database-time watermark, and a SHA-256 fingerprint of the tool scope and filters; an HMAC-SHA-256 envelope authenticates the complete payload. Deleting the prior page's anchor or updating mutable disagreement state does not invalidate traversal; rows whose immutable `created_at` is later than the first-page watermark are intentionally excluded. This is bounded watermark traversal, not a cross-request PostgreSQL MVCC snapshot: a transaction that began before the first page and commits afterward can still contribute a row when its `created_at` is at or before the watermark. Oversized, malformed, tampered, cross-tool, and changed-filter cursors return a sanitized `validation_error`. Cursors are API tokens and clients must not parse or manufacture them. Legacy requests that omit the new paging fields retain the bounded default page, and null request objects remain valid for `list_jobs` and `list_profiles`.

Document tools remain point operations (`store_document_file`, `get_document_metadata`, and extraction/generation by UUID); there is no document-list snapshot API or residual unbounded document listing in this change. If document listing is added later, it must adopt the same opaque snapshot/keyset policy rather than exposing raw UUID anchors.
...
./mvnw -q -Dtest=PdfTextExtractionServiceTests,DocumentStorageServiceTests,PdfGenerationServiceTests,GeneratePdfResumeServiceTests,OfflineFrenchResumeTranslatorTests,DocumentMcpAdapterTests,DocumentPdfGenerateResumeMcpTests,DocumentCaPdfGenerateResumeMcpTests,DocumentCanadianFrenchPdfGenerateResumeMcpTests test
./mvnw -q -Dtest=ProfilePdfIngestionServiceTests,ProfilePdfIngestionMcpAdapterTests test
./mvnw -q -Dtest=JobServiceTests,JobMcpAdapterTests,HttpJobLinkContentFetcherTests test
...
The GitHub Actions pipeline runs fast validation and unit tests on pull requests to `development`. Feature/fix pull requests targeting `master` additionally run the Docker-backed integration/coverage gate, containerized MCP STDIO smoke, and Qodana. A `development` → `master` promotion PR is a special case whose candidate tree must match `origin/development` exactly; the same promotion guard runs on direct pushes to `master` as defense in depth. Trusted pushes to `development` and manual CI runs also retain the heavy integration and container smoke gates. Successfully merging a same-repository `development` → `master` pull request automatically starts the release workflow; direct pushes, fork pull requests, other source branches, and closed-but-unmerged pull requests cannot publish. The workflow confirms the merge commit is still the current `origin/master` tip and tree-identical to the immutable head SHA recorded on the merged promotion pull request, so later legitimate changes on `development` do not invalidate the release. It then increments the patch component of the highest stable `vMAJOR.MINOR.PATCH` tag (ignoring prerelease tags). A rerun reuses a stable tag only when it already points to the exact verified source commit. Automatic promotions share a serialized concurrency group with `cancel-in-progress: false`; GitHub retains only one pending run per group, so rapid overlapping promotions may coalesce and only the newest pending promotion is attempted. Stale runs fail closed when their source is no longer the current `master` tip. Manual dry runs use a per-run group and cannot displace an automatic promotion. The workflow runs clean full verification (`./mvnw clean -Drevision=<version> -Pintegration-tests verify`), copies the verified Maven jar to a versioned release artifact, builds and smoke-tests `linux/amd64` and `linux/arm64` release images from that same verified jar, records and transfers each exact smoke-tested platform image, and generates CycloneDX and SPDX SBOMs for the jar and each image. Immediately before publication, it rechecks the verified source against current `master`, creates and pushes the annotated release tag idempotently, and publishes without relying on that bot-created tag push to trigger another workflow. The publishing job verifies each loaded image ID and the corresponding registry config digest before composing a GHCR multi-platform manifest list. Stable releases also publish `:latest`; the workflow verifies both required platforms, signs and attests the immutable manifest-list digest with keyless Cosign, then uploads the jar, checksum, per-platform SBOMs, image IDs/digests, and verification reports as release artifacts. Docker Desktop automatically selects the matching Linux architecture on macOS and Windows; the project does not publish `darwin/*` or Windows-container variants.
...
Releases are promotion-driven; do not create or push release tags manually during the normal release path. Merge a reviewed `development` → `master` pull request after its required checks pass, and the successful merge event performs the next stable patch release automatically.

To exercise the release flow safely without publishing anything, run `.github/workflows/release.yml` manually from a branch or `development`. The workflow dispatch path performs clean Maven verification, verified-jar packaging, builds and smoke-tests both Linux release architectures, and generates jar plus per-platform image SBOMs. It also exercises keyless Cosign signing and SLSA provenance attestation against `SHA256SUMS`, then verifies both with the expected GitHub Actions OIDC workflow identity. Manual dispatch always remains a dry run: it never creates a tag or GitHub Release and never publishes a GHCR package. Download the dry-run artifacts and verify the JAR checksum locally with:
