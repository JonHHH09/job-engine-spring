...
## Container release and versioning

The primary distributable package is the local-only MCP server container image
published to GitHub Container Registry (`ghcr.io`). GitHub Releases and GHCR
container packages are created only by the `Release MCP Container` workflow when
it runs on a pushed release tag. Ordinary pushes, pull requests, and manual
branch runs verify the project but do not publish release artifacts.

Use semantic release tags in `vMAJOR.MINOR.PATCH` format, such as `v0.1.0`.
Prerelease suffixes are allowed, such as `v0.1.0-rc.1`, but prerelease tags do
not update the `latest` container tag. The workflow strips the leading `v` and
builds Maven with that release version, so `v0.1.0` produces
`target/job-engine-spring-0.1.0.jar`, uploads
`job-engine-spring-v0.1.0.jar`, labels the release image as version `0.1.0`,
and publishes `ghcr.io/<owner>/job-engine-spring:v0.1.0`. Stable tags also
publish `ghcr.io/<owner>/job-engine-spring:latest`.

Before creating a release tag, verify the intended release commit:
...
Create and push an annotated release tag only after the release commit is ready:
...
After the tag push, monitor the `Release MCP Container` workflow in GitHub
Actions. A successful stable tag release creates:

- a GitHub Release named after the tag,
- the release jar as an attached artifact,
- `SHA256SUMS`,
- `IMAGE-ID`,
- `IMAGE-DIGESTS`,
- a GHCR container package with image tags for the exact release tag and
  `latest`.

The workflow authenticates to `ghcr.io` with `GITHUB_TOKEN`, and
`Dockerfile.release` sets `org.opencontainers.image.source` to the repository
URL so GitHub can associate the container package with this repository. The
first published package may be private by default; adjust package visibility in
GitHub Packages if the image should be public.

If GitHub Packages still does not show the container after a successful tag
release, check the package visibility and repository-linking settings in GitHub
Packages. CLI package listing may require a GitHub token with `read:packages`.
