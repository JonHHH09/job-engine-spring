...
CREATE INDEX job_search_terms_term_prefix_idx
    ON job_schema.search_terms (term, job_id, field_key) INCLUDE (weight);
...
CREATE INDEX profile_search_terms_term_prefix_idx
    ON profile.search_terms (term, profile_id, field_key) INCLUDE (weight);
...
CREATE INDEX reports_list_created_id_idx ON match.reports (created_at DESC, id);
CREATE INDEX reports_profile_created_id_idx ON match.reports (profile_id, created_at DESC, id);
CREATE INDEX reports_job_created_id_idx ON match.reports (job_id, created_at DESC, id);
